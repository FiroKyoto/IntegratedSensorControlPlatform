...
namespace Display
{
    partial class IntegratedForm
    {
        ...
        private void CombinePlayForClient()
        {
            ...
            if (this.client_map_flag == true)
            {
                double angle = 0.0;
                if (this.BodyModelComboBox.SelectedIndex == 0)
                {
                    angle = this._vy50.gps_Heading;
                }
                else if (this.BodyModelComboBox.SelectedIndex == 1)
                {
                    angle = this._vy446.gps_Compass;
                }

                this.clientMap.UpdateCurrentPosition(this.tmX, this.tmY, angle, this.BodyModelComboBox.SelectedIndex);
            }
        }
